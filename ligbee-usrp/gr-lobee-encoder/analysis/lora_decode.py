...
class LoRaDecode:
    ...
    def decode(self, symbols):
        ...
        self.print_hex(self.packet[0:self.length + 3], "After DH")
    ...
    def print_hex(self, data, opt):
        if self.debug:
            print(opt, end=': ')
            for d in data:
                print(hex(d).upper(), end=', ')
            print()
...
if __name__ == "__main__":
    ...
    test_length = 7
    # ground truth
    test_symbols = np.array([12, 4, 124, 124, 28, 28, 12, 4, 84, 94, 93, 85, 25, 116, 121, 116, 58, 27, 9, 38, 82, 118, 127, 47], np.uint16)
    # test_symbols = np.array([12, 4, 124, 124, 28, 28, 12, 4, 1, 94, 93, 85, 76, 116, 44, 33, 111, 27, 9, 38, 7, 118, 42, 122], np.uint16)
    # max correlation result
    # test_symbols = np.array([8, 0, 122, 120, 24, 24, 0, 0, 80, 88, 88, 80, 16, 112, 112, 112, 48, 16, 0, 32, 72, 112, 120, 40], np.uint16)
    # max gradient result
    # test_symbols = np.array([14, 4, 124, 124, 28, 28, 12, 4, 84, 94, 92, 84, 24, 114, 120, 114, 58, 26, 8, 36, 80, 116, 124, 46], np.uint16)
    # test_symbols = np.array([12, 4, 124, 124, 28, 28, 12, 1, 1, 92, 92, 84, 74, 114, 44, 32, 108, 24, 8, 36, 6, 116, 40, 120], np.uint16)
    # test_symbols = np.array([12, 4, 124, 124, 28, 28, 12, 4, 4, 96, 92, 84, 76, 116, 44, 36, 116, 28, 12, 36, 4, 120, 44, 124], np.uint16)
    # test_symbols = np.array([12, 4, 124, 124, 28, 28, 12, 1, 84, 94, 92, 84, 24, 114, 120, 114, 58, 26, 8, 36, 80, 116, 124, 44], np.uint16)
    # test_symbols = np.array([12, 4, 124, 124, 28, 28, 12, 4, 4, 12, 20, 28, 36, 44, 52, 60, 36, 44, 52, 60, 68, 76, 84, 92], np.uint16)
