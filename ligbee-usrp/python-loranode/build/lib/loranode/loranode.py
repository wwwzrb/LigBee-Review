...
class RN2483Controller(LoRaController):
    ...
    def recv_symbol(self, num):
        ...
        self.serial_sr(CMD_RX_RADIO, str(num)) # num (number) of symbols to be received
        r = self.serial_r()
        data = r[8:].strip()
    ...
    def set_sync(self, sync):
        self.serial_sr(CMD_MAC_PAUSE)
        self.serial_sr(CMD_SET_SYNC, str(sync))
        self.serial_sr(CMD_MAC_RESUME)
    ...
    def set_prlen(self, value):
        self.serial_sr(CMD_MAC_PAUSE)
        self.serial_sr(CMD_SET_PRLEN, str(value))
        self.serial_sr(CMD_MAC_RESUME)
    ...
    def get_wdt(self):
        return self.serial_sr(CMD_GET_WDT)

    def set_wdt(self, wdt):
        self.serial_sr(CMD_MAC_PAUSE)
        self.serial_sr(CMD_SET_WDT, str(wdt))
        self.serial_sr(CMD_MAC_RESUME)
